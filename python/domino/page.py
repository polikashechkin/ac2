...
class UserPageStore:
    def __init__(self, redis, account_id, module_id, user_id, page_id):
        self.redis = redis
        self.key = f'ctx|{account_id}|{module_id}|{user_id}|{page_id}'
        dump = self.redis.get(self.key)
        self.js = pickle.loads(dump) if dump else {}

    def __save(self):
        self.redis.set(self.key, pickle.dumps(self.js))
...
class Page:
 ...
 #       self._STORE = None
    ...
    @property
    def REDIS_1(self):
        if self._REDIS_1 is None:
            self._REDIS_1 = redis.Redis('localhost', port=6379, db=1)
        return self._REDIS_1
...
#            self._STORE = Store(self.redis, self.SK, self.ID)  
    ...
    def USER_PAGE_STORE(self):
        if self._USER_PAGE_STORE is None:
            self._USER_PAGE_STORE = UserPageStore(self.REDIS_1, self.account_id,  self.application.module_id, self.user_id, self.ID)
